...
@Composable
private fun CollectionSearchResultRow(product: SearchProduct, isCollected: Boolean, onClick: () -> Unit) {
    Surface(onClick = onClick, modifier = Modifier.fillMaxWidth()) {
        ListItem(
            ...
            trailingContent = {
                if (isCollected) {
                    AssistChip(onClick = onClick, label = { Text("Already in collection") })
                }
            },
        )
    }
